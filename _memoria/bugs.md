...
### [ABERTO] 2026-09-02 — apps/roleplay: trabalho de sessão anterior nunca commitado (login CRM, voz OpenAI)
- Sintoma: ao mexer no roleplay pra criar o modo "Reunião Única", `git status` mostrou modificações não commitadas em `App.tsx`, `api.ts`, `stt.ts`, `tts.ts`, `docker-compose.yml`, `server/package.json`, `server/src/index.ts`, `.env.example`, e os arquivos novos `server/src/auth.ts` e `server/src/routes/voice.ts` — nenhum deles tocado por mim nesta sessão.
- O conteúdo bate com a memória `nexo-treino-app-roleplay`: login via e-mail/senha do CRM e voz via OpenAI (`gpt-4o-mini-tts`/`gpt-4o-mini-transcribe`), que a memória descreve como **NO AR desde 05/08/2026**. Ou seja: está rodando em produção na VPS, mas nunca foi salvo no GitHub.
- Impacto: se a VPS cair ou alguém reclonar o repo pra redeploy, essas duas features (login e voz OpenAI) somem — o `git log` não sabe que elas existem.
- Correção: (pendente — decisão do Mario) commitar esse trabalho separado do commit do modo Reunião Única, pra manter o histórico rastreável. Não commitei sozinho porque a regra do sistema é só commitar quando o usuário pedir.

### [ABERTO] 2026-09-02 — apps/roleplay: persona gerada por nicho nunca recebe `voz`
- Sintoma: `server/src/claude/personaGen.ts` (`PERSONA_SCHEMA`/`generatePersona`) nunca pede o campo `voz` ao modelo, mas as 5 personas fixas (`server/src/personas/fixas.ts`) têm `voz` definida, e `Session.tsx` manda `persona.voz` direto pro TTS (`new BackendTTS((t) => api.tts(t, persona.voz))`).
- Impacto: toda persona criada por "✨ Gerar por nicho…" cai na voz fixa `'ash'` do servidor, independente de gênero/personalidade — quebra silenciosamente a variação de voz que as fixas têm.
- Achado pela revisão adversarial (`/code-review`) ao mexer no `personaGen.ts` pra adicionar o campo `varredura`; o bug em si é anterior a essa mudança e não foi corrigido agora (fora do escopo do modo Reunião Única).
- Correção: (pendente) adicionar `voz` (enum das vozes do TTS) ao `PERSONA_SCHEMA` e à instrução do gerador.
